...
#define smsc43340_phy_power_on(gpio)	gpio_export(gpio, 1)
#define smsc43340_phy_power_off(gpio)	gpio_export(gpio, 0)

static void smsc43340_phy_suspend(struct smsc43340_usb *smsc, int controller_off)
{
	if (smsc->asleep)
		return;

	if (smsc->otg.link_save_context)
		smsc->otg.link_save_context(&smsc->otg);

	smsc43340_phy_power_off(smsc->gpio_reset);
	smsc->asleep = 1;
}
...
static void smsc43340_phy_resume(struct smsc43340_usb *smsc)
{
	if (!smsc->asleep)
        ...
        smsc43340_phy_power_on(smsc->gpio_reset);
	smsc->asleep = 0;

	if (smsc->otg.link_restore_context)
		smsc->otg.link_restore_context(&smsc->otg);
}
...
static void smsc43340_irq_work_handler(struct work_struct *work)
{
	...
	struct otg_transceiver x = smsc->otg;
...
static int __init smsc43340_usb_probe(struct platform_device *pdev)
{
	...
	/* Reset USB Phy */
	gpio_set_value(smsc->gpio_reset, 0);
	mdelay(20);
	gpio_set_value(smsc->gpio_reset, 1);
