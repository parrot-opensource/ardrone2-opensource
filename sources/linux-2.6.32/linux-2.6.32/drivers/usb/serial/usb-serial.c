...
static int debug;
/* PARROT PATCH FOR GPS MODULE */
static bool gps_pl2303_device;
/* initially all NULL */
			       ...
			       const struct usb_device_id *id)
{
	...
	if (((le16_to_cpu(dev->descriptor.idVendor) == PL2303_VENDOR_ID) &&
		...
		/* PARROT PATCH FOR GPS MODULE */
		gps_pl2303_device = 1;
	} else /* PARROT PATCH FOR GPS MODULE */
		gps_pl2303_device = 0;

	/* END HORRIBLE HACK FOR PL2303 */
#endif

#if defined(CONFIG_USB_SERIAL_CP210X) || \
    defined(CONFIG_USB_SERIAL_CP210X_MODULE)
	if (((le16_to_cpu(dev->descriptor.idVendor) == 0x19CF) &&
	     (le16_to_cpu(dev->descriptor.idProduct) == 0x0888))) {
		gps_pl2303_device = 1; /* Parrot SiRF4 GPS module */
	}

	if (((le16_to_cpu(dev->descriptor.idVendor) == 0x19cf) &&
	     (le16_to_cpu(dev->descriptor.idProduct) == 0x3000))) {
		gps_pl2303_device = 1; /* Parrot Mykonos2 GPS module */
	}
#endif

	/* PARROT PATCH FOR GPS MODULE */
	if (gps_pl2303_device) {
	...
	if (serial->type->read_int_callback) {
		for (i = 0; i < num_interrupt_in; ++i) {
			if (gps_pl2303_device) {
				...
				/*
				 * update ep status
				 * DEBUG macro enables a check of urb type
				 */
