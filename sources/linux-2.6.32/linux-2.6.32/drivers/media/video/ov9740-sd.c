...
struct ov9740_sensor {
	...
	struct v4l2_subdev_frame_interval timeperframe;
	int scaler;
	int ver;
	int fps;
	bool resuming;
...
#define to_ov9740_sensor(sd)	container_of(sd, struct ov9740_sensor, subdev)

static unsigned long xclk_current = OV9740_XCLK_NOM_1;

/* list of image formats supported by ov9740 sensor */
static enum v4l2_mbus_pixelcode ov9740_formats[] = {
 ...
 * @fi: frame period numerator and denominator in seconds
 *
 * The maximum exposure time is also updated since it is affected by the
 * frame rate.
 **/
static int ov9740_set_framerate(struct v4l2_subdev *s,
				struct v4l2_subdev_frame_interval *fi)
{
	int err = 0;
	u16 isize = isize_current;
	u32 frame_length_lines, line_time_q8;
	struct ov9740_sensor *sensor = to_ov9740_sensor(s);
	struct ov9740_sensor_settings *ss;
	struct v4l2_fract *fper = &fi->interval;
 ...
 * returned.
 */
static int ov9740sensor_set_gain(u16 lineargain, struct v4l2_subdev *s,
				 struct vcontrol *lvc)
{
	int err = 0, i;
	u16 reg_gain = 0;
 ...
 * @isize: image size enum
 */
static int ov9740_setup_mipi(struct v4l2_subdev *s, unsigned isize)
{
#if 0
	struct ov9740_sensor *sensor = to_ov9740_sensor(s);
	struct i2c_client *client = sensor->i2c_client;
 ...
 * returned.
 */
static int ov9740_configure_test_pattern(int mode, struct v4l2_subdev *s,
					 struct vcontrol *lvc)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(s);
	struct i2c_client *client = sensor->i2c_client;
 ...
 */
static int ov9740_configure(struct v4l2_subdev *s)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(s);
	struct i2c_client *client = sensor->i2c_client;
 ...
 */
static int ioctl_queryctrl(struct v4l2_subdev *s, struct v4l2_queryctrl *qc)
{
	int i;
 ...
 * Otherwise, returns -EINVAL if the control is not supported.
 */
static int ioctl_g_ctrl(struct v4l2_subdev *s, struct v4l2_control *vc)
{
	struct vcontrol *lvc;
	int i;
 ...
 * Otherwise, * returns -EINVAL if the control is not supported.
 */
static int ioctl_s_ctrl(struct v4l2_subdev *s, struct v4l2_control *vc)
{
	int retval = -EINVAL;
	int i;
	struct vcontrol *lvc;
 ...
 */
static int ioctl_enum_fmt_cap(struct v4l2_subdev *s,
			      struct v4l2_fmtdesc *fmt)
{
	...
	fmt->pixelformat = ov9740_formats[index];
 ...
 */
static int ioctl_try_fmt_cap(struct v4l2_subdev *s, struct v4l2_format *f)
{
	unsigned isize;
	int ifmt;
	...
	for (ifmt = 0; ifmt < NUM_CAPTURE_FORMATS; ifmt++) {
		if (pix->pixelformat == ov9740_formats[ifmt])
			break;
	}
	if (ifmt == NUM_CAPTURE_FORMATS)
		ifmt = 0;
	pix->pixelformat = ov9740_formats[ifmt];
...
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(s);
	struct v4l2_subdev_frame_interval *timeperframe = &a->parm.capture.timeperframe;
	int err = 0;
...
static int __ov9740_power_off_standby(struct v4l2_subdev *s,
				      enum v4l2_power on)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(s);
	struct i2c_client *client = sensor->i2c_client;
...
static int ov9740_power_off(struct v4l2_subdev *s)
{
	return __ov9740_power_off_standby(s, V4L2_POWER_OFF);
}

static int ov9740_power_standby(struct v4l2_subdev *s)
{
	return __ov9740_power_off_standby(s, V4L2_POWER_STANDBY);
}

static int ov9740_power_on(struct v4l2_subdev *s)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(s);
	struct i2c_client *client = sensor->i2c_client;
	int rval;

	sensor->pdata->set_xclk(s, OV9740_XCLK_NOM_1);

	rval = sensor->pdata->set_power(s, V4L2_POWER_ON);
	if (rval < 0) {
		v4l_err(client, "Unable to set the power state: "
			OV9740_DRIVER_NAME " sensor\n");
		sensor->pdata->set_xclk(s, 0);
...
static int ioctl_enum_framesizes(struct v4l2_subdev *s,
				  ...
				  struct v4l2_subdev_frame_size_enum *frms)
{
	int ifmt;

	for (ifmt = 0; ifmt < NUM_CAPTURE_FORMATS; ifmt++) {
		if (frms->code == ov9740_formats[ifmt])
			break;
	}
...
static int ioctl_enum_frameintervals(struct v4l2_subdev *s,
				  struct v4l2_subdev_fh *fh,
				  struct v4l2_subdev_frame_interval_enum *frmi)
{
	int ifmt;

	/* Check that the requested format is one we support */
	for (ifmt = 0; ifmt < NUM_CAPTURE_FORMATS; ifmt++) {
		if (frmi->code == ov9740_formats[ifmt])
			break;
	}
	...
	frmi->interval.numerator =
				ov9740_frameintervals[frmi->index].numerator;
	frmi->interval.denominator =
...
static int ov9740_enum_mbus_code(struct v4l2_subdev *subdev,
				 struct v4l2_subdev_fh *fh,
...
static int ov9740_enum_frame_size(struct v4l2_subdev *subdev,
				  struct v4l2_subdev_fh *fh,
...
static int ov9740_enum_frame_ival(struct v4l2_subdev *subdev,
				  struct v4l2_subdev_fh *fh,
...
__ov9740_get_pad_format(struct ov9740_sensor *sensor, struct v4l2_subdev_fh *fh,
...
static int ov9740_get_pad_format(struct v4l2_subdev *subdev,
				 struct v4l2_subdev_fh *fh,
				 struct v4l2_subdev_format *fmt)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(subdev);
	struct v4l2_mbus_framefmt *format;

	format = __ov9740_get_pad_format(sensor, fh, fmt->pad, fmt->which);
	if (format == NULL)
		return -EINVAL;
...
static int ov9740_set_pad_format(struct v4l2_subdev *subdev,
				 struct v4l2_subdev_fh *fh,
				 struct v4l2_subdev_format *fmt)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(subdev);
...
static int ov9740_get_frame_interval(struct v4l2_subdev *subdev,
				     struct v4l2_subdev_frame_interval *fi)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(subdev);
...
static int ov9740_set_frame_interval(struct v4l2_subdev *subdev,
				     struct v4l2_subdev_frame_interval *fi)
{
	struct ov9740_sensor *sensor = to_ov9740_sensor(subdev);
...
static const struct v4l2_subdev_pad_ops ov9740_pad_ops = {
	...
	.enum_frame_interval = ov9740_enum_frame_ival,
				  ...
				  const struct i2c_device_id *id)
{
	...
	v4l2_i2c_subdev_init(&sensor->subdev, client, &ov9740_ops);
...
static int __exit ov9740_remove(struct i2c_client *client)
{
	...
	if (sensor->power_count) {
		if (sensor->pdata->set_power)
			sensor->pdata->set_power(&sensor->subdev,
							     0);
