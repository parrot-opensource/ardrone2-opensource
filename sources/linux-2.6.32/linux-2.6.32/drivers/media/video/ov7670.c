...
{
	return ov7670_write_array(sd, ov7670_default_regs_dev);
}
...
static int ov7670_enum_mbus_fmt(struct v4l2_subdev *sd, int index,
...
static int ov7670_frame_rates[] = { 60, 30, 15, 10, 5, 1 };
...
static int ov7670_pad_enum_mbus_code(struct v4l2_subdev *subdev,
				 struct v4l2_subdev_fh *fh,
				 struct v4l2_subdev_mbus_code_enum *code_enum)
{
	if (code_enum->index >= ARRAY_SIZE(ov7670_formats))
		return -EINVAL;

	code_enum->pad  = 0;

	return ov7670_enum_mbus_fmt(subdev, code_enum->index, &code_enum->code);
}

static int ov7670_pad_enum_frame_size(struct v4l2_subdev *subdev,
				  struct v4l2_subdev_fh *fh,
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->index >= ARRAY_SIZE(ov7670_formats))
		return -EINVAL;

	if (fse->code != ov7670_formats[fse->index].mbus_code)
		return -EINVAL;

	memset(&fsize, 0, sizeof (fsize));

	fsize.index = fse->index;

	ret = ov7670_enum_framesizes(subdev, &fsize);
	if (ret)
		return ret;

	fse->max_height = fse->min_height = fsize.discrete.height;
	fse->max_width  = fse->min_width = fsize.discrete.width;

	return 0;
}

static struct v4l2_mbus_framefmt *
__ov7670_pad_get_format(struct ov7670_info *sensor, struct v4l2_subdev_fh *fh,
			unsigned int pad, enum v4l2_subdev_format_whence which)
{
	...
	case V4L2_SUBDEV_FORMAT_ACTIVE:
		return &sensor->mbus_format;
	default:
		return NULL;
	}
}

static int ov7670_pad_get_format(struct v4l2_subdev *subdev,
				 struct v4l2_subdev_fh *fh,
				 struct v4l2_subdev_format *fmt)
{
	struct ov7670_info *sensor = to_state(subdev);
	struct v4l2_mbus_framefmt *format;

	format = __ov7670_pad_get_format(sensor, fh, fmt->pad, fmt->which);
	if (format == NULL)
		return -EINVAL;
...
static int ov7670_pad_set_format(struct v4l2_subdev *subdev,
				 struct v4l2_subdev_fh *fh,
				 struct v4l2_subdev_format *fmt)
{
	struct ov7670_info *sensor = to_state(subdev);
	...
	ret = ov7670_try_fmt_internal(subdev, &fmt->format, NULL, NULL);
	if (ret)
		return ret;
	...
	*format = fmt->format;
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_ACTIVE) {
		...
		ret = ov7670_s_mbus_fmt(subdev, format);

		if (!ret)
			sensor->mbus_format = fmt->format;
	}
...
static int ov7670_pad_enum_frame_ival(struct v4l2_subdev *subdev,
				  struct v4l2_subdev_fh *fh,
				  struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	for (cnt = 0; cnt < ARRAY_SIZE(ov7670_formats); cnt++) {
		if (fie->code == ov7670_formats[cnt].mbus_code)
			break;
	}

	if (cnt == ARRAY_SIZE(ov7670_formats))
		return -EINVAL;

	memset(&interval, 0, sizeof (interval));

	interval.index = fie->index;

	ret = ov7670_enum_frameintervals(subdev, &interval);
	if (ret)
		return ret;

	fie->interval.denominator = interval.discrete.denominator;
	fie->interval.numerator   = interval.discrete.numerator;
...
{
	struct ov7670_info *sensor = to_state(subdev);
	struct ov7670_platform_data *pdata = sensor->pdata;
	int ret = 0;
	...
	if (sensor->power_count == !on) {
		if (on) {
			...
			pdata->set_xclk(subdev, pdata->clock_speed);
			mdelay(5);
			ov7670_init(subdev,0);
		} else {
...
static int ov7670_registered(struct v4l2_subdev *subdev)
{
	...
	ret = ov7670_set_power(subdev, 1);
	if (ret) {
		v4l_warn(client, "Power on failed\n");
		return ret;
	}

	ret = ov7670_detect(subdev);

	ov7670_set_power(subdev, 0);
...
{
	struct ov7670_info *sensor = to_state(sd);
	struct v4l2_mbus_framefmt *format;

	format = __ov7670_pad_get_format(sensor, fh, 0, V4L2_SUBDEV_FORMAT_TRY);
	if (!format)
		return -EINVAL;
	...
	ov7670_try_fmt_internal(sd, format, NULL, NULL);
...
static const struct v4l2_subdev_pad_ops ov7670_pad_ops = {
	.enum_mbus_code 	= ov7670_pad_enum_mbus_code,
	.enum_frame_size 	= ov7670_pad_enum_frame_size,
	.enum_frame_interval 	= ov7670_pad_enum_frame_ival,
	.get_fmt 		= ov7670_pad_get_format,
	.set_fmt 		= ov7670_pad_set_format,
};
			...
			const struct i2c_device_id *id)
{
	...
	pdata = (struct ov7670_platform_data *) client->dev.platform_data;
	...
	v4l2_i2c_subdev_init(&info->sd, client, &ov7670_ops);

	/*
	 * Must apply configuration before initializing device, because it
	 * selects I/O method.
	 */
	info->min_width = info->pdata->min_width;
	info->min_height = info->pdata->min_height;
	info->use_smbus = info->pdata->use_smbus;
	info->clock_speed = info->pdata->clock_speed;

	format = __ov7670_pad_get_format(info, NULL, 0,
	...
	ov7670_try_fmt_internal(&info->sd, format, NULL, NULL);
	...
	info->sd.internal_ops = &ov7670_internal_ops;
...
static int ov7670_remove(struct i2c_client *client)
{
	...
	if (sensor->power_count && sensor->pdata) {
		if (sensor->pdata->set_power)
			sensor->pdata->set_power(&sensor->sd, 0);

		sensor->pdata->set_xclk(&sensor->sd, 0);
