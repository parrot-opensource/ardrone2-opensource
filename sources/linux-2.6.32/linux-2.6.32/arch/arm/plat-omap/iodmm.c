...
EXPORT_SYMBOL_GPL(add_mapping_info);
...
EXPORT_SYMBOL_GPL(proc_begin_dma);
...
EXPORT_SYMBOL_GPL(proc_end_dma);
...
EXPORT_SYMBOL_GPL(user_remove_resources);

static int __init iodmm_init(void)
{
	return 0;
}
module_init(iodmm_init);

static void __exit iodmm_exit(void)
{
}
module_exit(iodmm_exit);
